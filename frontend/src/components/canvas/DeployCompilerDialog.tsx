...
export function DeployCompilerDialog({
  ...
  const creates =
    review?.operations.filter((op) => op.kind === "createVm") ?? []
        ...
        <AlertDialog.Popup className="fixed left-1/2 top-1/2 z-50 flex h-[min(760px,calc(100vh-2rem))] w-[min(1180px,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl border bg-popover text-popover-foreground shadow-2xl ring-1 ring-foreground/10">
          <header className="flex items-start justify-between gap-4 border-b px-6 py-4">
            <div>
              ...
              <AlertDialog.Description className="mt-1 text-xs text-muted-foreground">
                The backend rebuilt the dependency graph from the final
                topology. Select a step to highlight its canvas resources.
              </AlertDialog.Description>
            </div>
            <div className="flex shrink-0 items-center gap-4 text-[11px] text-muted-foreground">
              <span className="flex items-center gap-1.5">
                <Clock3 className="h-3.5 w-3.5" />
                {duration(review?.estimatedDurationSeconds ?? 0)} total
              </span>
              <span className="flex items-center gap-1.5">
                <GitBranch className="h-3.5 w-3.5" />
                {duration(review?.criticalPathDurationSeconds ?? 0)} critical
              </span>
            </div>
          </header>
                ...
                <ul className="mt-3 space-y-2">
                  {diagnostics.map((item) => (
                    <li
                      ...
                      className="flex gap-2 rounded-lg border p-2.5 text-[11px] leading-snug"
                    >
                      <AlertTriangle
                        className={cn(
              ...
              <dl className="mt-2 grid grid-cols-2 gap-2 text-xs">
                <div className="rounded-lg border p-2">
                  ...
                  <dd className="mt-1 text-lg font-semibold">
                    {review?.resources.nodes}
                  </dd>
                </div>
                <div className="rounded-lg border p-2">
                  <dt className="text-muted-foreground">Links</dt>
                  <dd className="mt-1 text-lg font-semibold">
                    {review?.resources.relationships}
                  </dd>
                </div>
                <div className="col-span-2 rounded-lg border p-2">
                  <dt className="text-muted-foreground">DNS records</dt>
                  <dd className="mt-1 text-lg font-semibold">
                    {review?.resources.dnsRecords.length}
                  </dd>
                </div>
              </dl>
            </section>
                ...
                {review?.operations.map((op, index) => {
                  ...
                  return (
                    <li key={op.id}>
                      <button
                        type="button"
                        onClick={() => highlight(op.id)}
                        className={cn(
                          "group flex w-full gap-3 rounded-xl border p-3 text-left transition-colors hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
                          critical.has(op.id) &&
                      ...
                      >
                        ...
                        <span className="min-w-0 flex-1">
                          ...
                          <span className="mt-0.5 block truncate text-[11px] text-muted-foreground">
                            {node?.data.name ?? op.target}
                            {op.secondary
                              ? ` ← ${nodes.find((item) => item.id === op.secondary)?.data.name ?? op.secondary}`
                              : ""}
                          </span>
                          <span className="mt-1 block text-[10px] text-muted-foreground">
                            {op.dependsOn.length === 0
                              ? "Starts immediately"
                              : `After ${op.dependsOn.map((id) => kindLabel(operationById.get(id)?.kind ?? id)).join(", ")}`}
                          </span>
                        </span>
                      </button>
                    </li>
            ...
            <section className="overflow-y-auto p-4">
                  ...
                  <li key={op.id} className="rounded-lg border p-3 text-[11px]">
                    ...
                    <dl className="mt-2 grid grid-cols-[56px_1fr] gap-x-2 gap-y-1 text-muted-foreground">
                      ...
                      <dd className="truncate">
                        Role-mapped {op.params.template}
                      </dd>
                      <dt>IP</dt>
                      <dd>Next free guest-pool address</dd>
                      <dt>Action</dt>
                      <dd>Clone and verify</dd>
                    </dl>
                  </li>
                ))}
              </ul>
              ...
              {secretFields.length === 0 ? (
                <p className="mt-2 text-[11px] text-muted-foreground">
                  No secret-bearing fields are referenced.
                </p>
              ) : (
                <ul className="mt-2 space-y-1 text-[11px] text-muted-foreground">
                  {secretFields.map((field) => (
                    <li key={field} className="rounded-md border px-2 py-1.5">
                      {field} · masked
                    </li>
                  ))}
                </ul>
          ...
          <footer className="flex items-center justify-between gap-4 border-t px-6 py-4">
            <p className="text-[11px] text-muted-foreground">
              Critical path: {review?.criticalPath.length ?? 0} operations ·
              topology v{review?.topologyVersion}
            </p>
            <div className="flex gap-2">
              ...
              <Button size="sm" onClick={onConfirm}>
                Deploy {creates.length} VMs / {review?.operations.length ?? 0}{" "}
                verified steps
              </Button>
            </div>
          </footer>
