...
export function isCertificateJourney(
  value: unknown,
): value is CertificateJourney {
...
export function projectCertificateJourney(
  ...
  const parent = edges.find(
    (edge) =>
      ...
      edge.target === issuing.id,
  )
  const membership = edges.find(
    (edge) =>
      edge.data?.edgeType === EDGE_TYPE.domainJoin && edge.source === web.id,
  )
  const dc = nodes.find((node) => node.id === membership?.target)
  const nodeIds = Array.from(
    new Set([web.id, issuing.id, ...(parent ? [parent.source] : [])]),
  )
  const ocsp = edges.find(
    (edge) =>
      edge.data?.edgeType === EDGE_TYPE.webServerCert &&
  ...
  if (web.data.certificateJourney) {
    return {
      journey: web.data.certificateJourney,
      nodeIds,
      edgeIds,
      live: true,
    }
  ...
  const dns = (hostname: string, address?: string) => ({
    hostname,
  ...
  return {
    ...
    journey: {
      ...
      hops: [
        hop(
          "enroll",
          `${web.data.name} probe enrolls`,
          `adcs://${caHost}/Workstation`,
          dns(caHost, issuing.data.ip),
          ["lab-health-probe.cer"],
        ),
        hop(
          "issue",
          `${issuing.data.name} issues`,
          `adcs://${caHost}/${issuing.data.config?.commonName ?? issuing.data.name}`,
          dns(caHost, issuing.data.ip),
          ["lab-health-probe.cer"],
        ),
        hop(
          "aia",
          "AIA builds the chain",
          `http://${pkiHost}/CertEnroll/`,
          dns(pkiHost, web.data.ip),
          ["issuing CA certificate", "root CA certificate"],
        ),
        hop(
          "cdp",
          "CDP checks revocation",
          `http://${pkiHost}/CertEnroll/`,
          dns(pkiHost, web.data.ip),
          ["base CRLs", "delta CRL"],
        ),
